...
assert.equal(demoSlides.length, 12, "the V2 product demo must contain twelve scenes");
assert.deepEqual(demoSlides.map((match) => Number(match[1])), Array.from({ length: 12 }, (_, index) => index + 1), "V2 scene numbers must be sequential");
assert.equal(narrationScenes.length, 12, "the V2 product demo must contain one narration entry per scene");
