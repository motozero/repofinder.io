...
async function run(repoUrl, goal) {
  ...
  lastGoal = goal;
  setBusy(true);
  showStatus("Reading the project and searching live GitHub data. This usually takes a few seconds.");
...
function render(data) {
  lastMode = data.mode || "github-fallback";
  if (data.source) {
    sourceEl.innerHTML = `
      <div class="source-head"><h2>${escapeHtml(data.source.fullName)}</h2><span class="mode ${lastMode === "openai" ? "ai" : "fallback"}">${lastMode === "openai" ? "OpenAI ranked" : "GitHub fallback"}</span></div>
  ...
  const method = lastMode === "openai" ? "OpenAI ranked" : "GitHub ranked";
  showStatus(`${method} ${recs.length} candidates for “${data.goal}”.`);
  resultsEl.innerHTML = recs.map((repo, index) => card(repo, index, lastMode === "openai")).join("");
}

function card(r, i, chatEnabled) {
  return `
    <article class="card">
      <div class="head">
      ...
      ${chatEnabled ? `<button type="button" class="chat-btn" data-repo="${escapeAttr(r.fullName)}">Ask OpenAI about this repo</button>` : ""}
    </article>`;
}
