...
Open the repository and show that the browser API and `recommend_repos` MCP tool call one engine. Then show the checked-in Codex skill. MCP supplies the capability. The skill supplies repeatable workflow judgment.
...
Show the strict schemas, deterministic tests, model evals, SSRF checks, Cloudflare resource isolation, and public lesson book. End with: “The demo is the visible surface. The reusable accelerator is the architecture and the operating discipline behind it.”
