...
function loadEnv(): { OPENAI_API_KEY: string; GITHUB_TOKEN?: string } {
  ...
  return { OPENAI_API_KEY: env.OPENAI_API_KEY, GITHUB_TOKEN: env.GITHUB_TOKEN || undefined };
}
...
async function runCase(c: EvalCase, env: ReturnType<typeof loadEnv>): Promise<CaseReport> {
  ...
  for (let t = 0; t < TRIALS; t++) {
    ...
    if (!NO_JUDGE) judges.push((await judgeFit(c, result, env.OPENAI_API_KEY)).score);
  }
