...
import { callOpenAI, parseStructured, MODELS, type JsonSchemaFormat } from "../src/openai.ts";
...
// contract the engine promises (see src/engine.ts and AGENTS.md). If one of
...
/** House style (AGENTS.md): no em dashes or doubled hyphens in user-facing copy. */
...
//
// Caveat worth knowing: using the same model family for generation and grading
// can introduce preference bias. JUDGE_MODEL is separate and easy to change.
// ---------------------------------------------------------------------------

export const JUDGE_MODEL = MODELS.reason;

const judgeFormat: JsonSchemaFormat = {
  name: "recommendation_scores",
  description: "Fit scores for each repository recommendation.",
  schema: {
    type: "object",
    properties: {
      scores: {
        ...
        items: {
          ...
          required: ["fullName", "score", "reason"],
...
export async function judgeFit(
  ...
  const parsed = parseStructured<{ scores: { fullName: string; score: number; reason: string }[] }>(text);
